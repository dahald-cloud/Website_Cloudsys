...
require_once dirname(__DIR__) . '/includes/site-pages.php';
...
try {
    if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
        ...
        if ($action === 'pages') {
            $selected = is_array($_POST['visible_pages'] ?? null) ? $_POST['visible_pages'] : [];
            $allowed = array_keys(cloudsys_managed_pages());
            foreach ($selected as $slug) if (!is_string($slug) || !in_array($slug, $allowed, true)) throw new DomainException('Choose valid public pages.');
            $database = cloudsys_db(); $database->beginTransaction();
            $statement = $database->prepare("INSERT INTO site_settings (setting_key, setting_value, updated_by) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), updated_by = VALUES(updated_by), updated_at = CURRENT_TIMESTAMP");
            foreach ($allowed as $slug) $statement->execute(['page_visible_' . $slug, in_array($slug, $selected, true) ? '1' : '0', (int) $admin['id']]);
            $database->commit();
            $message = 'Public page visibility updated successfully.';
        } elseif ($action === 'chat') {
        ...
        } else throw new DomainException('Choose a valid settings action.');
    }
    $currentMode = cloudsys_chat_mode();
    $pageVisibility = cloudsys_page_visibility();
    ...
    $currentMode = 'admins';
    $pageVisibility = array_fill_keys(array_keys(cloudsys_managed_pages()), true);
}
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <link rel="stylesheet" href="/style.css?v=20260907-1" />
  <link rel="stylesheet" href="/admin-styles.css?v=20260907-1" />
</head>
<body class="admin-dashboard-shell">
  ...
  <main class="admin-dashboard">
    ...
    <section class="admin-panel page-visibility-panel"><p class="admin-eyebrow">PUBLIC PAGES</p><h2>Page visibility</h2><p>Hidden pages are removed from navigation and sitemaps. Their direct URLs return the branded not-found page.</p><form method="post" action="/admin/"><input type="hidden" name="csrf" value="<?= htmlspecialchars(cloudsys_csrf_token(), ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="action" value="pages"><div class="page-visibility-grid"><?php foreach (cloudsys_managed_pages() as $slug => $page): ?><label><input type="checkbox" name="visible_pages[]" value="<?= htmlspecialchars($slug, ENT_QUOTES, 'UTF-8') ?>" <?= ($pageVisibility[$slug] ?? true) ? 'checked' : '' ?>><span><strong><?= htmlspecialchars($page['label'], ENT_QUOTES, 'UTF-8') ?></strong><small><?= htmlspecialchars($page['path'], ENT_QUOTES, 'UTF-8') ?></small></span></label><?php endforeach; ?></div><button class="admin-save" type="submit">Save page visibility <span>&rarr;</span></button></form></section>
