...
try {
    ...
    $query = cloudsys_db()->prepare('SELECT a.id, a.title, a.status, a.published_at, a.updated_at, c.name AS category_name FROM articles a JOIN article_categories c ON c.id=a.category_id ORDER BY a.updated_at DESC, a.id DESC LIMIT 20 OFFSET ?');
...
<link rel="stylesheet" href="/article-admin.css?v=20260922-1">
...
<p>Total: <?= (int) $counts['total'] ?> · Drafts: <?= (int) $counts['drafts'] ?> · Scheduled: <?= (int) $counts['scheduled'] ?> · Public: <?= (int) $counts['published'] ?></p>
<div class="article-create-actions"><a class="admin-primary-link article-create-link" href="/admin/edit-article.php">Create manually <span>→</span></a><a class="admin-primary-link article-create-link article-ai-link" href="/admin/ai-writer.php">Write with AI <span>✦</span></a></div>
...
<?php foreach ($rows as $row): $displayStatus = $row['status'] === 'published' && $row['published_at'] > gmdate('Y-m-d H:i:s') ? 'scheduled' : $row['status']; ?><tr><td><?= cloudsys_article_escape($row['title']) ?></td><td><?= cloudsys_article_escape($row['category_name']) ?></td><td><?= cloudsys_article_escape($displayStatus) ?></td><td><div class="article-list-actions"><a href="/admin/edit-article.php?id=<?= (int) $row['id'] ?>">Edit</a><a href="/admin/preview-article.php?id=<?= (int) $row['id'] ?>" target="_blank" rel="noopener">Preview</a></div></td></tr><?php endforeach; ?>
