...
## Admin AI article studio

Back up the database, then import `deployment/migrate-article-ai.sql` after the Insights migrations. It creates an admin-scoped audit record for each generation and is safe to run repeatedly. Add `ARTICLE_AI_MODEL`, `ARTICLE_AI_FALLBACK_MODEL`, and `ARTICLE_AI_MAX_OUTPUT_TOKENS` to the private `cloudsys-config.php` outside `public_html`; the writer reuses the existing server-side `OPENROUTER_API_KEY` and exact `https://openrouter.ai/api/v1` base URL. Suggested values are shown in `deployment/cloudsys-config.example.php`.

Signed-in administrators can open `/admin/ai-writer.php`, provide a topic, audience, purpose, approved facts, source excerpts, editorial restrictions, and an optional JPEG/PNG reference image. Briefs and reference images are sent to OpenRouter with zero-data-retention and data-collection-denial routing requested. OpenRouter/provider behavior and legal terms remain external dependencies, so disclose this processing in the privacy notice and never submit client secrets or personal data without an approved basis.

Generation produces structured, validated article fields, SEO metadata, fact-check notes, image suggestions, and a readiness state. It does not fetch URLs or publish automatically. The administrator must review the rendered result and then explicitly import an editable draft, publish it, schedule it at least five minutes ahead in UTC, or discard it. Results with review notes or `needs-review` readiness cannot bypass the editable-draft path. The audit table records the administrator, model, brief, generated output, token counts, state, timestamps, and resulting article. Each administrator sees only their own generation history. Generation is limited to ten requests per administrator per hour.

Before production use, run PHP 8.3 lint on `includes/article-ai.php` and `admin/ai-writer.php`, run `node --check article-ai.js`, and test with a low-risk factual brief. Confirm missing API configuration fails closed; an invalid upload is rejected; draft import remains private; fact-check notes block direct publication; scheduling excludes the article until its UTC time; discard cannot later be imported; repeated submissions cannot reuse one generation; and the OpenRouter account accepts the configured models and requested privacy routing. Review the first several articles closely for factual accuracy, tone, links, and claims.
